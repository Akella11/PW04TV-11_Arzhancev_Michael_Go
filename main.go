...
func calculateTask1(I, t, Sm, jEk float64) string {
	...
	output := fmt.Sprintf(
		"Розрахунковий струм для нормального режиму: %.2f A\n"+
			"Розрахунковий струм для післяаварійного режиму: %.2f A\n"+
			"Економічний переріз: %.2f мм^2\n"+
			"Оптимальний переріз: %.2f мм^2\n",
		Im,
		ImPA,
		sEk,
		s,
...
func calculateTask3(Rcn, Xcn, RcMin, XCMin float64) string {
	SNom := 6.3
	Uh := 115.0
	Ul := 11.0
	UkMax := 11.1
...
func calculator3Handler(w http.ResponseWriter, r *http.Request) {
	...
	result := calculateTask3(input.Values[0], input.Values[1], input.Values[2], input.Values[3])
